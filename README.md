...
### Data Ingestion
- First step will be to read the data from mongoDB, second is to create the feature storeand the third step will be to split the Train and Test data and finally save it in the ingested folder. 
...
```
- Now we create another class like we have created for Data Ingestion and in that we can specify what all output we want from data Ingestion component, data validation component etc.
```bash
...
- In the root folder, main.py will be created which will have the following logic:
```bash
...
- Also we'll need to create a class for Data-Validation in artifact_entity.py. Basically we need the output of that particular data-validation config in artifact_entity as it will values such as validation status, valid train file path, valid test file path etc.
```bash
@dataclass
...
- In constant > training_pipeline > __inti__.py add these:
```bash
"""
Data Validation related constant start with DATA_VALIDATION VAR NAME
"""
